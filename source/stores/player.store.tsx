...
export function playerReducer(
	...
	switch (action.category) {
		...
		case 'STOP':
			...
			// Keep currentTrack so the user can resume/replay it with Space after
			// stopping. Only halt playback and reset progress here; the mpv process
			// is torn down by the play/pause effect reacting to isPlaying=false.
			return {
				...state,
				isPlaying: false,
				progress: 0,
	...
	useEffect(() => {
		...
		const trackIdentity =
			track.source === 'local' && track.localPath
				? track.localPath
				: track.videoId;
		const isSameTrack = currentTrackId === trackIdentity;
		...
		const loadAndPlayTrack = async () => {
			...
			const isLocalTrack = track.source === 'local' && !!track.localPath;
			...
			for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
				try {
					...
					// Pass YouTube URL directly to mpv (it handles stream extraction via
					// yt-dlp). Local tracks pass their filesystem path instead.
					const youtubeUrl =
						track.source === 'local' && track.localPath
							? track.localPath
							: `https://www.youtube.com/watch?v=${track.videoId}`;
