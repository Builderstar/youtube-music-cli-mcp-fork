...
import {isLocalMediaPath} from '../../utils/local-media.ts';
...
class PlayerService {
	...
	async play(url: string, options?: PlayOptions): Promise<void> {
		...
		// Detect a local file path/URL so it is passed to mpv untouched.
		const isLocalSource = isLocalMediaPath(url);

		// Extract videoId from URL (YouTube only). For local files use the path
		// itself as the identity so the "same track" guard still works.
		const videoIdMatch = url.match(/[?&]v=([^&]+)/);
		const videoId = isLocalSource ? url : videoIdMatch ? videoIdMatch[1] : null;
		...
		// Build YouTube URL from a bare videoId if needed. Local file paths and
		// anything with an explicit scheme (http(s)://, file://) are left intact
		// so mpv's loadfile can resolve them directly.
		let playUrl = url;
		if (!isLocalSource && !url.startsWith('http')) {
