...
class MusicService {
	...
	async getTrack(videoId: string): Promise<Track | null> {
		...
		try {
			const yt = await getClient();
			const info = (await yt.music.getInfo(normalizedVideoId)) as unknown as {
				basic_info?: {
					id?: string;
					title?: string;
					author?: string;
			...
			// basic_info.author is the channel name; for YouTube Music tracks this
			// is typically "Artist - Topic", so strip the "- Topic" suffix.
			const rawArtistName = basic.author?.trim();
			const artistName = rawArtistName
				? rawArtistName.replace(/\s*-\s*Topic$/i, '').trim()
				: '';

			if (title) {
				return {
					videoId: basic.id?.trim() || normalizedVideoId,
					title,
					artists: artistName
						? [{artistId: basic.channel_id?.trim() || '', name: artistName}]
						: [],
					duration:
			...
			logger.warn('MusicService', 'getTrack returned no title', {
				videoId: normalizedVideoId,
			});
		} catch (error) {
			logger.error('MusicService', 'getTrack failed', {
				videoId: normalizedVideoId,
				error: error instanceof Error ? error.message : String(error),
			});
		}

		// Fallback so callers (MCP add/download) still get a persistable track.
		return {
			videoId: normalizedVideoId,
			title: 'Unknown Track',
			artists: [],
