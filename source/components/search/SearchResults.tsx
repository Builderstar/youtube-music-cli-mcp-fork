...
function SearchResults({
	...
	const openPlaylistPicker = useCallback(() => {
		if (!isActive || pickerTrack) return;
		const selected = results[selectedIndex];
		if (selected && selected.type === 'song') {
			setPickerTrack(selected.data as Track);
		}
	}, [isActive, pickerTrack, results, selectedIndex]);

	useKeyBinding(KEYBINDINGS.UP, navigateUp);
	useKeyBinding(KEYBINDINGS.DOWN, navigateDown);
	useKeyBinding(KEYBINDINGS.SELECT, handleSelect);
	useKeyBinding(KEYBINDINGS.ADD_TO_PLAYLIST, openPlaylistPicker);
		...
		<Box flexDirection="column">
			{pickerTrack && (
				<PlaylistPicker
					...
					onDone={message => {
						setPickerTrack(null);
						if (message) downloadStatusRef.current?.(message);
					}}
				/>
			)}
			{/* Results list */}
