...
const SETTINGS_ITEMS = [
	...
	'Manage Plugins',
	'Local Music Folder',
	'Include Downloads in Local Music',
] as const;
...
export default function Settings() {
	...
	const [localMusicDirectory, setLocalMusicDirectory] = useState(
		config.get('localMusicDirectory') ?? '',
	);
	const [localMusicIncludeDownloads, setLocalMusicIncludeDownloads] = useState(
		config.get('localMusicIncludeDownloads') ?? true,
	);
	const [isEditingLocalMusicDirectory, setIsEditingLocalMusicDirectory] =
		useState(false);
	...
	// While any text field is being edited, block the global/settings command
	// keybindings so characters like '/', '-', 'c', etc. are typed into the
	// TextInput instead of triggering shortcuts (mirrors the search field).
	const isEditingField =
		isEditingApiKey ||
		isEditingDownloadDirectory ||
		isEditingBaseUrl ||
		isEditingLocalMusicDirectory;
	useKeyboardBlocker(isEditingField);
	...
	const cancelEditing = () => {
		...
		setIsEditingLocalMusicDirectory(false);
		// Restore field values from persisted config (discard uncommitted edits).
		setDownloadDirectory(config.get('downloadDirectory') ?? '');
		setLocalMusicDirectory(config.get('localMusicDirectory') ?? '');
		setLLMApiKey(config.getLLMApiKey() ?? '');
		setLLMBaseUrl(config.getLLMConfig()?.baseUrl ?? '');
	};

	// Esc cancels editing (bypasses the keyboard block, like the search field).
	useKeyBinding(KEYBINDINGS.BACK, cancelEditing, {bypassBlock: true});
	...
	const toggleLocalMusicIncludeDownloads = () => {
		const next = !localMusicIncludeDownloads;
		setLocalMusicIncludeDownloads(next);
		config.set('localMusicIncludeDownloads', next);
	};
	...
	const handleSelect = () => {
		if (selectedIndex === 0) {
			...
			setIsEditingLocalMusicDirectory(true);
		} else if (selectedIndex === 24) {
			toggleLocalMusicIncludeDownloads();
		}
	};
	...
	return (
		<Box flexDirection="column" gap={1}>
			...
			{/* Local Music Folder */}
			<Box paddingX={1}>
				{isEditingLocalMusicDirectory && selectedIndex === 23 ? (
					<TextInput
						value={localMusicDirectory}
						onChange={setLocalMusicDirectory}
						onSubmit={value => {
							const normalized = value.trim();
							setLocalMusicDirectory(normalized);
							config.set('localMusicDirectory', normalized);
							setIsEditingLocalMusicDirectory(false);
						}}
						placeholder="Local music directory (empty to disable)"
						focus
					/>
				) : (
					<Text
					...
					>
						Local Music Folder: {localMusicDirectory || '(not set)'}
					</Text>
				)}
			</Box>
			...
			<Box paddingX={1}>
				...
				>
					Include Downloads in Local Music:{' '}
					{localMusicIncludeDownloads ? 'ON' : 'OFF'}
				</Text>
			</Box>
