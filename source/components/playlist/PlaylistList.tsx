...
export default function PlaylistList() {
	...
	const openPlaylist = useCallback(() => {
		...
		if (!playlist) return;
		dispatch({
			category: 'SET_ACTIVE_PLAYLIST',
			playlistId: playlist.playlistId,
		});
		dispatch({category: 'NAVIGATE', view: VIEW.PLAYLIST_DETAIL});
	}, [dispatch, playlists, selectedIndex, renamingPlaylistId]);
	...
	useKeyBinding(KEYBINDINGS.SELECT, openPlaylist);
	useKeyBinding(KEYBINDINGS.OPEN_PLAYLIST, openPlaylist);
	useKeyBinding(KEYBINDINGS.CREATE_MIX, startPlaylist);
	...
	return (
		<Box flexDirection="column" gap={1}>
			...
			<Box marginTop={1}>
				<Text color={theme.colors.dim}>
					<Text color={theme.colors.text}>Enter</Text> open |{' '}
					<Text color={theme.colors.text}>m</Text> play |{' '}
