...
export default function PlayerControls() {
	...
	const handlePlayPause = () => {
		...
		// If mpv is no longer alive (e.g. after a Stop or the process ended), a
		// plain resume has nothing to un-pause. Replay the current track from the
		// start instead so Space reliably restarts a stopped song.
		if (
			playerState.currentTrack &&
			!getPlayerService().hasActivePlaybackSession()
		) {
			dispatch({category: 'PLAY', track: playerState.currentTrack});
			return;
		}
