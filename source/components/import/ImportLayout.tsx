...
import {getM3UImportService} from '../../services/import/m3u-import.service.ts';
...
const SOURCES: Array<{key: ImportSource; label: string}> = [
	...
	{key: 'm3u', label: 'Local M3U/M3U8 file'},
];
...
export default function ImportLayout() {
	...
	const startImport = useCallback(async () => {
		...
		try {
			...
			if (source === 'm3u') {
				const m3uService = getM3UImportService();
				const importResult = await m3uService.importFromFile(
					url,
				...
				<Box flexDirection="column" gap={1}>
					<Text color={theme.colors.dim}>
						{SOURCES[selectedSource]!.key === 'm3u'
							? 'Enter path to a local .m3u / .m3u8 file:'
					...
					<Text color={theme.colors.dim}>
						...
						{SOURCES[selectedSource]!.key === 'spotify'
							...
							: SOURCES[selectedSource]!.key === 'youtube'
								? 'https://www.youtube.com/playlist?list=...'
								: '~/Music/mymix.m3u8 or /home/user/list.m3u'}
					</Text>
				</Box>
			)}
